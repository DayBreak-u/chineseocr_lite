...
#include "psenet_lite_mbv2.id.h"
#include "crnn_lite_lstm_v2.id.h"
...
class OCR
{
    public:
        ...
        OCR(AAssetManager *mgr,const std::vector<ncnn::Mat> param_files, const std::vector<ncnn::Mat> bin_files);
        ~OCR();
        std::vector<std::vector<int>> Detect(cv::Mat im_bgr,int long_size);
        ...
        void pse_deocde(ncnn::Mat & features,
                         ...
                         const float ratio );
    std::vector<int> crnn_deocde(const ncnn::Mat score , std::vector<std::string> alphabetChinese);
