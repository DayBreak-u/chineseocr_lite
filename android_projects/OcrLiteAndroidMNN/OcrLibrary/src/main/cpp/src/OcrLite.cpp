...
void OcrLite::init(JNIEnv *jniEnv, jobject assetManager, int numThread,jstring dbnetmodelPath ,jstring angelmodelPath, jstring crnnmodelPath) {
    AAssetManager *mgr = AAssetManager_fromJava(jniEnv, assetManager);
    if (mgr == NULL) {
        LOGE(" %s", "AAssetManager==NULL");
    }

    const char *dbnet = jniEnv->GetStringUTFChars(dbnetmodelPath, 0);
    const char *angel = jniEnv->GetStringUTFChars(angelmodelPath, 0);
    const char *crnn = jniEnv->GetStringUTFChars(crnnmodelPath, 0);

    Logger("--- Init DbNet ---\n");
    dbNet.setNumThread(numThread);
    dbNet.initModel(dbnet);

    Logger("--- Init AngleNet ---\n");
    angleNet.setNumThread(numThread);
    angleNet.initModel(angel);

    Logger("--- Init CrnnNet ---\n");
    crnnNet.setNumThread(numThread);
    crnnNet.initModel(mgr,crnn);
